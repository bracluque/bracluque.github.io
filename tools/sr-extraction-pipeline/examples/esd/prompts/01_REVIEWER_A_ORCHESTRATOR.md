...
This design exists because extracting 245+ papers in one session blows up the context window. Each worker handles a 5-paper chunk and returns a compact summary; your context stays small.
...
This is roughly 4× faster than sequential dispatch. With 8 papers per worker × 4 parallel workers = 32 papers in flight per wave.
...
Process this batch of papers for Reviewer A extraction. For each one, read the PDF, extract per the skill files, and save to Output\ESD\_reviewerA\{ID}.md.
...
After all chunks are dispatched, print a final summary:
...
- **Do NOT extract papers yourself.** You dispatch, you do not do the work. Your role is lightweight coordination.
...
- **Default: parallel waves of 4 workers, 8 papers each (32 papers per wave).** Adjust upward only if user asks; drop to sequential only if user asks.
- **Wait for the entire wave to complete before launching the next.** Do not interleave.
