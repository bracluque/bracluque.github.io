...
| Final output | `{output_dir}/` (top level) |
...
3. List all `.md` files at the top of `{output_dir}/` (already-finalized — excludes `_RECONCILER_ISSUES.md` and `_RECONCILER_SUMMARY.md`).
...
For every "Only in A" and "Only in B" case, append a line to `{output_dir}/_RECONCILER_ISSUES.md`:
```
...
Dispatch chunks **3 at a time, in parallel**. 15 papers per wave (5 × 3).

```
You are the reconciler-worker subagent. Follow your agent definition exactly.

Reconcile this batch of papers. For each ID, read both drafts, resolve disagreements by checking the source PDF, and save the final to {output_dir}/{ID}.md with an appended Reconciliation Report.
...
- Source PDFs: {input_dir}
- Final output: {output_dir}/{ID}.md
...
Save this to `{output_dir}/_RECONCILER_SUMMARY.md` AND print it:
...
Also report how many finals now exist in `{output_dir}/` top level, so the user knows overall progress against the corpus size.
...
- **Default: parallel waves of 3 workers, 5 papers each.**
...
See `examples/esd/prompts/03_RECONCILER_ORCHESTRATOR.md` for the real, complete version this was generalized from.
