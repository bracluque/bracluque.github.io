...
| Input | Path |
|-------|------|
| Reviewer A draft | `{output_dir}/_reviewerA/{ID}.md` |
| Reviewer B draft | `{output_dir}/_reviewerB/{ID}.md` |
| Source PDF | `{input_dir}/*{ID}*.pdf` |

---

## Output
`{output_dir}/{ID}.md` — consolidated final file, same template as reviewers, plus a **Reconciliation Report** section at the bottom.
...
| **Trivially differ** | e.g., one decimal place, formatting differences, quote wording slightly different but coded value identical. Carry the cleaner version. No re-check needed. |
...
#### Coding-scheme fields
Disagreements here often come from ambiguous descriptions in the paper. Re-read the relevant section. If genuinely ambiguous, pick the best-fitting code and record the ambiguity in the Reconciliation Report.
...
#### Outcomes / measures lists
A common source of drift: one reviewer lists something that is "mentioned" but not "measured and reported". Only measured-and-reported items go in the final list.

#### Notes and prose summaries
Merge bullets from both reviewers (deduplicate). For any linear prose summary, write a fresh consolidated paragraph that captures both reviewers' points.
...
|---------|-------|------------------|------------------|-------------|-----------------------------|-----------|
| [section] | [field] | [A's value] | [B's value] | [Final] | "…" (p. X) | [One sentence on why this value wins] |
| … | … | … | … | … | … | … |
...
- [ ] File saved to `{output_dir}/{ID}.md` (not in a subfolder)
